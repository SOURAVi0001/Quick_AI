...
const LinkedinOptimizer = () => {
    ...
    return (
      <div className="mb-6 border border-border/50 rounded-xl overflow-hidden bg-card">
        <div className="bg-muted/30 p-5 border-b border-border/50">
          ...
          <div className="mb-2">
            <h4 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-3">Current</h4>
        ...
        <div className="p-5 bg-accent/5">
          ...
          {alternatives && alternatives.length > 0 && (
            <div className="mt-5 space-y-3">
              <h4 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Alternatives</h4>
              {alternatives.map((alt, idx) => (
                <div key={idx} className="flex justify-between items-center bg-background/50 p-4 rounded-lg border border-border/30">
  ...
  const PostResultBlock = ({ original, analysis, recommended }) => {
    ...
    return (
      <div className="mb-6 border border-border/50 rounded-xl overflow-hidden bg-card">
         <div className="bg-muted/30 p-5 border-b border-border/50">
          <h4 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-3">Original Post</h4>
    ...
    return (
      <div className="mb-6 border border-border/50 rounded-xl p-5 bg-card">
        ...
        <div className="flex gap-2 mb-4">
          <span className="text-[11px] font-medium px-2.5 py-1 bg-muted/50 rounded text-muted-foreground">Topic: {topic}</span>
        </div>
        ...
        <div className="bg-accent/5 border border-border/30 p-5 rounded-lg">
          <div className="flex justify-between items-center mb-3">
            <h5 className="text-[11px] font-bold text-accent uppercase tracking-widest">Suggested Post</h5>
              ...
              <form onSubmit={onSubmitHandler} className="space-y-4">
                <div className="space-y-5 border-b border-border/50 pb-6">
                  <div className="space-y-2.5">
                    ...
                    <Input name="targetRole" value={formData.targetRole} onChange={handleInputChange} placeholder="e.g., Senior Frontend Engineer" required className="bg-background border-border/50 focus-visible:ring-accent" />
                  </div>
                  <div className="space-y-2.5">
                    <label className="text-sm font-medium text-foreground">Optimization Goal <span className="text-destructive">*</span></label>
                    <Input name="optimizationGoal" value={formData.optimizationGoal} onChange={handleInputChange} placeholder="e.g., Get more recruiter messages" required className="bg-background border-border/50 focus-visible:ring-accent" />
                  </div>
                  <div className="space-y-2.5">
                    <label className="text-sm font-medium text-foreground">Current Headline <span className="text-destructive">*</span></label>
                    <Textarea name="headline" value={formData.headline} onChange={handleInputChange} placeholder="Your current LinkedIn headline" required className="bg-background border-border/50 focus-visible:ring-accent resize-none" rows={2} />
                  </div>
                </div>

                <div className="space-y-5 pt-4 pb-6 border-b border-border/50">
                  <h3 className="text-sm font-semibold text-foreground tracking-wide">Optional Sections</h3>
                  
                  <div className="space-y-2.5">
                    <label className="text-sm font-medium text-muted-foreground">About / Summary</label>
                    <Textarea name="about" value={formData.about} onChange={handleInputChange} rows={3} placeholder="Paste your current about section" className="bg-background border-border/50 focus-visible:ring-accent resize-y" />
                  </div>
                  <div className="space-y-2.5">
                    <label className="text-sm font-medium text-muted-foreground">Experience</label>
                    <Textarea name="experience" value={formData.experience} onChange={handleInputChange} rows={3} placeholder="Paste your experience entries" className="bg-background border-border/50 focus-visible:ring-accent resize-y" />
                  </div>
                  <div className="space-y-2.5">
                    <label className="text-sm font-medium text-muted-foreground">Projects</label>
                    <Textarea name="projects" value={formData.projects} onChange={handleInputChange} rows={2} placeholder="Paste key projects" className="bg-background border-border/50 focus-visible:ring-accent resize-y" />
                  </div>
                  <div className="space-y-2.5">
                    <label className="text-sm font-medium text-muted-foreground">Skills</label>
                    <Input name="skills" value={formData.skills} onChange={handleInputChange} placeholder="e.g., React, Node.js, AWS" className="bg-background border-border/50 focus-visible:ring-accent" />
                  </div>
                </div>
                ...
                <div className="space-y-5 pt-4">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-sm font-semibold text-foreground tracking-wide">Recent Posts</h3>
                      <p className="text-[11px] text-muted-foreground mt-1">Add up to 10 recent posts for analysis</p>
                    </div>
                    <div className="flex gap-2">
                      {posts.length > 0 && (
                        <Button type="button" variant="ghost" size="sm" onClick={clearPosts} className="h-8 text-xs">Clear</Button>
                      )}
                      <Button type="button" variant="outline" size="sm" onClick={addPost} disabled={posts.length >= 10} className="h-8 gap-1.5 text-xs border-border/50">
                    ...
                    <div className="space-y-3 mt-4">
                      {posts.map((post, idx) => (
                        <div key={idx} className="flex gap-3 items-start">
                          <span className="text-xs font-mono text-muted-foreground pt-3 w-4">{idx + 1}.</span>
                          <Textarea 
                            value={post} 
                          ...
                          />
                          <Button type="button" variant="ghost" size="icon" onClick={() => removePost(idx)} className="text-destructive hover:text-destructive hover:bg-destructive/10 shrink-0">
                ...
                <Button type="submit" disabled={loading} className="w-full mt-8 h-12 bg-accent text-accent-foreground hover:bg-accent/90 shadow-md shadow-accent/20">
                  {loading ? (
            ...
            <Card className="h-full border-dashed border-border/50 bg-transparent">
              <CardContent className="h-full flex flex-col items-center justify-center min-h-[400px] text-center p-8">
                ...
                <p className="text-sm text-muted-foreground max-w-md leading-relaxed">
                  Fill out your LinkedIn profile information on the left and click Analyze Profile to get AI-powered recommendations ready to copy and paste.
                </p>
              </CardContent>
            </Card>
          ) : (
              ...
              <div className="grid grid-cols-2 gap-4">
                <Card className="bg-card border-border/50 overflow-hidden">
                  <div className="p-6 relative">
                    ...
                    <p className="text-muted-foreground text-[11px] font-bold uppercase tracking-widest mb-2">Overall Score</p>
                    <div className="flex items-baseline gap-1">
                      <div className="text-4xl font-bold text-foreground">{results.overallScore || 0}</div>
                ...
                <Card className="bg-card border-border/50 overflow-hidden">
                  <div className="p-6 relative">
                    ...
                    <p className="text-muted-foreground text-[11px] font-bold uppercase tracking-widest mb-2">Role Alignment</p>
                    <div className="flex items-baseline gap-1">
                      <div className="text-4xl font-bold text-foreground">{results.roleAlignmentScore || 0}</div>
                      <div className="text-sm font-medium text-muted-foreground">/100</div>
                    </div>
                  </div>
                </Card>
                ...
                {results.skills && (
                  <Card className="bg-card border-border/50">
                    ...
                    <CardContent className="pt-6 space-y-6">
                      <div>
                        <div className="flex justify-between items-center mb-3">
                          <h4 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Recommended Order</h4>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {results.skills.recommendedOrder && results.skills.recommendedOrder.map((s, i) => (
                            <span key={i} className="px-3 py-1 bg-accent/10 text-accent text-xs font-medium rounded-full border border-accent/20">{s}</span>
                          ))}
                        </div>
                      </div>
                      ...
                      {results.skills.missingSkills && results.skills.missingSkills.length > 0 && (
                        <div>
                          <div className="flex justify-between items-center mb-3">
                            <h4 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest">Missing Skills to Add</h4>
                            <CopyButton text={results.skills.missingSkills.join(', ')} className="h-8" />
                          </div>
                          <div className="flex flex-wrap gap-2">
                            {results.skills.missingSkills.map((s, i) => (
                              <span key={i} className="px-3 py-1 bg-orange-500/10 text-orange-500 text-xs font-medium rounded-full border border-orange-500/20">{s}</span>
                            ))}
                          </div>
                        </div>
                      )}
          ...
          return (
            <div className="p-6">
              ...
              <div className="bg-background/50 rounded-lg p-4 border border-border/30">
                <p className="text-sm text-foreground font-medium mb-2">Target Role: {content.targetRole || 'Not specified'}</p>
