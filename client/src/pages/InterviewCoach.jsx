  ...
  const handleStart = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setChatHistory([{ type: 'ai', text: data.firstQuestion }]);
            ...
            <div>
              ...
              <ul className="space-y-1">
                {evaluation.weaknesses.map((w, i) => (
                  <li key={i} className="flex gap-2 items-start">
                    <span className="text-muted-foreground mt-0.5">•</span> <span>{w}</span>
                  </li>
                ))}
              </ul>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <Input
                      ...
                      required
                      className="bg-background border-border/50 h-12 focus-visible:ring-accent"
                    />
                  </div>
                  <div className="space-y-3">
                    <label className="text-sm font-medium text-foreground">
                ...
                <div className="p-6">
                  ...
                  <div className="bg-background/50 rounded-lg p-4 border border-border/30">
                    ...
                    <ul className="text-xs text-muted-foreground space-y-1 mb-3">
                      {content.strongAreas?.slice(0, 2).map((s, i) => (
                    ...
                    <ul className="text-xs text-muted-foreground space-y-1">
                      {content.weakAreas?.slice(0, 2).map((w, i) => (
                        <li key={i} className="line-clamp-1">
                          • {w}
                        </li>
                      ))}
        ...
        <Card className="flex flex-col h-[700px] max-h-[85vh] shadow-xl border-accent/20 animate-in fade-in zoom-in-95 duration-300">
          <CardHeader className="border-b border-border/50 py-4 px-6 flex flex-row items-center justify-between bg-card">
            <div>
              ...
              <p className="text-xs text-accent mt-1 tracking-wide font-medium">
              ...
              {isConcluded && overallFeedback && (
                <div className="w-full mt-12 bg-card border border-accent/30 rounded-xl overflow-hidden shadow-lg animate-in slide-in-from-bottom-8 duration-700">
                  <div className="bg-accent/10 p-6 text-center border-b border-accent/20">
                    ...
                    <div className="text-5xl font-black text-accent mt-4 drop-shadow-sm">
                      {overallFeedback.overallScore}
                      <span className="text-2xl text-accent/50">/10</span>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-3 gap-6 text-center">
                      <div className="bg-background/50 p-4 rounded-xl border border-border/50">
                        ...
                        <div className="text-2xl font-bold text-foreground">
                          {overallFeedback.technicalScore}
                      ...
                      <div className="bg-background/50 p-4 rounded-xl border border-border/50">
                        ...
                        <div className="text-2xl font-bold text-foreground">
                          {overallFeedback.communicationScore}
                          <span className="text-sm text-muted-foreground">/10</span>
                        </div>
                      </div>
                      <div className="bg-background/50 p-4 rounded-xl border border-border/50">
                        ...
                        <div className="text-2xl font-bold text-foreground">
                          {overallFeedback.structureScore}
                          <span className="text-sm text-muted-foreground">/10</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-accent/5 p-6 rounded-xl border border-accent/20 text-sm">
                      ...
                      <ul className="space-y-2 text-foreground/90">
                        {overallFeedback.recommendedPractice?.map((item, i) => (
                          <li key={i} className="flex gap-2 items-start">
                            <span className="text-accent/50 mt-0.5">•</span> <span>{item}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="space-x-3 w-full sm:w-auto flex justify-end">
                    <Button
                      ...
                      className="border-border/50 h-9 px-4"
                    >
                      Conclude Interview
                    </Button>
                    <Button
                      ...
                      size="sm"
                      className="gap-2 h-9 px-6 bg-accent text-accent-foreground hover:bg-accent/90"
                    >
