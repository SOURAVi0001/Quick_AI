    ...
    <div className="border border-border/50 rounded-xl overflow-hidden mb-6 bg-card">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border/50">
        <div className="p-5 bg-background/50">
          <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest block mb-3">Current</span>
          <p className="text-sm text-muted-foreground whitespace-pre-wrap leading-relaxed">{current || "N/A"}</p>
        </div>
        <div className="p-5 bg-accent/5">
          ...
          <p className="text-sm text-foreground whitespace-pre-wrap font-medium leading-relaxed">{recommended}</p>
        </div>
      </div>
    </div>
  );
    ...
    <div className="pb-16 max-w-4xl mx-auto">
          ...
          <form onSubmit={handleGenerate} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <label className="text-sm font-medium text-foreground">Current Resume <span className="text-destructive">*</span></label>
                <Input 
                  type="file" 
                  accept=".pdf"
      ...
      {results && (
        <div className="mb-16 animate-in slide-in-from-bottom-4 duration-500 fade-in space-y-8">
            ...
            <div className="flex flex-col md:flex-row items-center p-8 gap-8">
              <div className="relative w-36 h-36 flex-shrink-0 flex items-center justify-center rounded-full bg-background/50 shadow-inner border border-accent/20">
                <svg className="absolute top-0 left-0 w-full h-full transform -rotate-90">
                  <circle cx="72" cy="72" r="68" stroke="currentColor" strokeWidth="8" fill="transparent" className="text-muted/30" />
                  <circle cx="72" cy="72" r="68" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray="427.2" strokeDashoffset={427.2 - (427.2 * results.matchScore) / 100} className="text-accent transition-all duration-1000" />
                </svg>
                <div className="z-10 flex flex-col items-center">
                  <span className="text-4xl font-bold text-foreground">{results.matchScore}</span>
                  <span className="text-[10px] font-bold text-accent uppercase tracking-widest mt-1">JD Match</span>
                </div>
              </div>
              ...
              <div className="flex-1 space-y-4 text-center md:text-left">
                ...
                <div className="flex flex-wrap gap-3 justify-center md:justify-start pt-2">
                  <div className="bg-green-500/10 text-green-500 border border-green-500/20 px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-1.5">
                    <Check className="w-3.5 h-3.5" /> {results.matchingSkills?.length || 0} Skills Matched
                  </div>
                  <div className="bg-destructive/10 text-destructive border border-destructive/20 px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-1.5">
                    <AlertTriangle className="w-3.5 h-3.5" /> {results.missingKeywords?.length || 0} Missing Keywords
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="bg-card">
              <CardHeader className="pb-3 border-b border-border/50">
                <CardTitle className="text-sm font-semibold text-foreground uppercase tracking-wider">Matching Skills</CardTitle>
              </CardHeader>
              <CardContent className="pt-4">
                <div className="flex flex-wrap gap-2">
                  {results.matchingSkills?.map((skill, i) => (
                    <span key={i} className="bg-green-500/10 text-green-500 border border-green-500/20 px-2.5 py-1 rounded text-[11px] font-medium tracking-wide">✓ {skill}</span>
                  ))}
            ...
            <Card className="bg-card">
              <CardHeader className="pb-3 border-b border-border/50">
                <CardTitle className="text-sm font-semibold text-foreground uppercase tracking-wider">Missing Keywords</CardTitle>
              </CardHeader>
              <CardContent className="pt-4">
                <div className="flex flex-wrap gap-2">
                  {results.missingKeywords?.map((kw, i) => (
                    <span key={i} className="bg-destructive/10 text-destructive border border-destructive/20 px-2.5 py-1 rounded text-[11px] font-medium tracking-wide">✗ {kw}</span>
                  ))}
                  {results.underEmphasizedKeywords?.map((kw, i) => (
                    <span key={`ue-${i}`} className="bg-orange-500/10 text-orange-500 border border-orange-500/20 px-2.5 py-1 rounded text-[11px] font-medium tracking-wide">⚠ {kw}</span>
                  ))}
                </div>
              </CardContent>
            </Card>
          ...
          <Card className="bg-card">
            <CardHeader className="pb-3 border-b border-border/50">
              <CardTitle className="text-sm font-semibold text-foreground uppercase tracking-wider">Experience Gaps & ATS Fixes</CardTitle>
            </CardHeader>
            <CardContent className="pt-5 grid md:grid-cols-2 gap-8">
              <div>
                <h5 className="text-[11px] font-bold text-muted-foreground mb-3 uppercase tracking-widest flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5 text-orange-500" /> Experience Gaps</h5>
                <ul className="space-y-2 text-sm text-foreground/80">
                  {results.experienceGaps?.map((gap, i) => <li key={i} className="flex gap-2 items-start"><span className="text-muted-foreground mt-0.5">•</span> <span>{gap}</span></li>)}
                </ul>
              </div>
              <div>
                <h5 className="text-[11px] font-bold text-muted-foreground mb-3 uppercase tracking-widest flex items-center gap-1.5"><Check className="w-3.5 h-3.5 text-green-500" /> ATS Recommendations</h5>
                <ul className="space-y-2 text-sm text-foreground/80">
                  {results.atsRecommendations?.map((rec, i) => <li key={i} className="flex gap-2 items-start"><span className="text-muted-foreground mt-0.5">•</span> <span>{rec}</span></li>)}
                </ul>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="bg-background/50 rounded-lg p-4 border border-border/30">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {content.missingKeywords?.slice(0, 5).map((kw, i) => (
                    <span key={i} className="text-[10px] bg-muted px-2 py-0.5 rounded text-muted-foreground">{kw}</span>
                  ))}
