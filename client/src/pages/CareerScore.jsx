  ...
  if (loading) {
    return (
      <div className="pb-16 max-w-6xl mx-auto flex flex-col items-center justify-center min-h-[600px]">
        ...
        <p className="text-sm font-medium animate-pulse text-foreground/80">Analyzing your career profile...</p>
      </div>
    );
  }
  ...
  return (
    <div className="pb-16 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        />
        <Button variant="outline" onClick={fetchScore} disabled={loading} className="shrink-0 sm:mt-0 -mt-4 mb-4 sm:mb-0 bg-card border-border/50 text-foreground hover:bg-muted/50">
        ...
        {!scoreData ? (
          <Card className="text-center p-12 bg-transparent border-dashed border-border/50">
            ...
            <p className="text-sm text-muted-foreground max-w-md mx-auto leading-relaxed">
              We couldn't find enough data to generate your career score. Try using the Resume Review, LinkedIn Optimizer, or AI Interview Coach first.
            </p>
          </Card>
        ) : (
            ...
            <Card className="lg:col-span-1 bg-card border-border/50 text-center flex flex-col items-center justify-center p-8 relative overflow-hidden">
              ...
              <div className="relative w-48 h-48 rounded-full flex items-center justify-center mb-8 bg-background border-8 border-background shadow-[0_0_30px_rgba(0,0,0,0.5)] z-10">
                <svg className="absolute top-0 left-0 w-full h-full transform -rotate-90">
                  <circle cx="88" cy="88" r="82" stroke="currentColor" strokeWidth="8" fill="transparent" className="text-muted/30" />
                  <circle cx="88" cy="88" r="82" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray="515" strokeDashoffset={515 - (515 * scoreData.overallScore) / 100} className={`${getScoreColor(scoreData.overallScore)} drop-shadow-md`} style={{ transition: 'stroke-dashoffset 1s ease-out' }} />
                </svg>
                <div className="flex flex-col items-center justify-center z-10">
                  <span className={`text-6xl font-black tracking-tighter ${getScoreColor(scoreData.overallScore)}`}>{scoreData.overallScore}</span>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mt-1">/ 100</span>
                </div>
              </div>
              
              <div className="w-full bg-accent/5 p-4 rounded-xl border border-accent/10 relative z-10">
                <p className="text-sm font-medium text-foreground/90">
                  Target Score: <span className="font-bold text-accent">{Math.min(100, scoreData.overallScore + 6)}</span>
                </p>
                <p className="text-xs text-muted-foreground mt-1">Complete recommended actions to improve.</p>
              </div>
            </Card>
                  ...
                  return (
                    <div key={cat} className="space-y-3">
                      <div className="flex justify-between items-center text-sm">
                        ...
                        {val ? (
                          <span className={`font-bold ${getScoreColor(val)}`}>{val}<span className="text-[10px] text-muted-foreground ml-0.5">/100</span></span>
                        ) : (
                          <span className="text-muted-foreground/50 italic text-[11px] uppercase tracking-wider font-semibold">Not analyzed</span>
                        )}
                      </div>
                      <Progress 
                        value={val || 0} 
                ...
                <CardContent className="pt-5">
                  <ul className="space-y-3">
                    {scoreData.strengths?.map((str, i) => (
                      <li key={i} className="flex gap-2.5 items-start text-sm text-foreground/90 leading-relaxed">
                        <CheckCircle2 className="w-4 h-4 text-green-500 shrink-0 mt-0.5" /> <span>{str}</span>
                      </li>
                    ))}
                    {(!scoreData.strengths || scoreData.strengths.length === 0) && <li className="text-sm text-muted-foreground italic">No strengths identified yet.</li>}
                  </ul>
                </CardContent>
              </Card>
                ...
                <CardContent className="pt-5">
                  <ul className="space-y-3">
                    {scoreData.weaknesses?.map((wk, i) => (
                      <li key={i} className="flex gap-2.5 items-start text-sm text-foreground/90 leading-relaxed">
                        <AlertTriangle className="w-4 h-4 text-orange-500 shrink-0 mt-0.5" /> <span>{wk}</span>
                      </li>
                    ))}
                    {(!scoreData.weaknesses || scoreData.weaknesses.length === 0) && <li className="text-sm text-muted-foreground italic">No areas to improve identified yet.</li>}
                  </ul>
                </CardContent>
              </Card>
                ...
                <CardContent className="pt-5">
                  <ul className="space-y-4">
                    {scoreData.recommendations?.map((rec, i) => (
                      <li key={i} className="flex gap-3 text-sm text-foreground/90 bg-background/50 p-3 rounded-lg border border-border/50 shadow-sm items-start">
                        <span className="font-bold text-accent bg-accent/10 px-2 py-0.5 rounded text-[10px] shrink-0 mt-0.5">+{rec.impact}</span>
                        <span className="leading-relaxed">{rec.action}</span>
                      </li>
                    ))}
                    {(!scoreData.recommendations || scoreData.recommendations.length === 0) && <li className="text-sm text-muted-foreground italic">No recommendations available.</li>}
                  </ul>
                </CardContent>
              </Card>
